...
# for METHOD in "${METHODS[@]}"; do
	# echo "2 ${METHOD}"
# 
	# RESULTS=$(psql -U postgres -d rundb -t -c "SELECT values FROM data WHERE name='gdconcmeasure' AND values->'method'='\"${METHOD}\"' ORDER BY timestamp DESC LIMIT 1");
	# 
...
FITSJSON=$(psql -U postgres -d rundb -At -c "SELECT row_to_json(r) FROM (SELECT values->'method' AS method,values->'absfit_success' AS absfit, (values->'peak_heights'->0)::float - (values->'peak_heights'->1)::float AS peakdiff, values->'conc_and_err'->0 as gdconc FROM data WHERE name='gdconcmeasure' ORDER BY timestamp DESC) AS r LIMIT 3")
FITSJSON=$(echo $FITSJSON | tr ' ' ',')
