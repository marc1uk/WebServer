...
async function UpdatePlot(name){
	...
	if(name=="dark_subtracted_data"){
		...
		let intraceUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=dark_subtracted_data_in&b=${debug}`;
		let outtraceUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=dark_subtracted_data_out&b=${debug}`;
		// we'll also overlay the original pure, and the result of the pure fitted to the data
		let pureTraceUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=dark_subtracted_pure&b=${debug}`;
		let pureFittedUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=pure_scaled&b=${debug}`;
...
//		console.log("outtrace.x is ",outtrace.x);
		...
		let dataUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=absorbance_trace&b=${debug}`;
		let fitUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=absfit&b=${debug}`;
		...
		let dataUrl = `/cgi-bin/marcus/get_latest_trace.cgi?a=${name}&b=${debug}`;
...
function check_for_new_data(name) {
	...
	let getTimeUrl = "";
	if(name=="dark_subtracted_data"){
		getTimeUrl = `/cgi-bin/marcus/get_last_trace_time.cgi?a=dark_subtracted_data_in&b=${debug}`;
	} else {
		getTimeUrl = `/cgi-bin/marcus/get_last_trace_time.cgi?a=${name}&b=${debug}`;
	}
